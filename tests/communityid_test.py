...
class TestCommunityID(unittest.TestCase):
    ...
    def assertEqualID(self, cft, correct_results):
        ...
        for cid, correct_res in cid_result_pairs:
            res = cid.calc(cft)
            self.assertEqual(
                res, correct_res, msg="%s: %s result is %s, should be %s, err: %s" % (cid, cft, res, correct_res, cid.get_error())
            )
    ...
    def verify_full_tuples(self, tuples, high_level_func, proto_num, af_family):
        ...
        for tpl in tuples:
            ...
            # Using a mix, ewww.
            cft = communityid.FlowTuple(
                proto_num, socket.inet_pton(af_family, tpl[0]), socket.inet_pton(af_family, tpl[1]), tpl[2], tpl[3]
            )
            self.assertEqualID(cft, tpl[4:])
            ...
            try:
                cft = communityid.FlowTuple(
                    proto_num, communityid.compat.ip_address(tpl[0]), communityid.compat.ip_address(tpl[1]), tpl[2], tpl[3]
                )
                self.assertEqualID(cft, tpl[4:])
            except RuntimeError:
                pass
    ...
    def verify_short_tuples(self, tuples, high_level_func, proto_num, af_family):
        ...
        for tpl in tuples:
            ...
            # Using packed NBO, as when grabbing from a packet header:
            cft = communityid.FlowTuple(proto_num, socket.inet_pton(af_family, tpl[0]), socket.inet_pton(af_family, tpl[1]))
            self.assertEqualID(cft, tpl[2:])

            # Using a mix, ewww.
            cft = communityid.FlowTuple(proto_num, tpl[0], socket.inet_pton(af_family, tpl[1]))
            self.assertEqualID(cft, tpl[2:])

            # Using Python 3.3+'s ipaddress types or their 2.x
            # backport:
            try:
                cft = communityid.FlowTuple(proto_num, communityid.compat.ip_address(tpl[0]), communityid.compat.ip_address(tpl[1]))
                self.assertEqualID(cft, tpl[2:])
            except RuntimeError:
                pass
    ...
    def test_inputs(self):
        ...
        with self.assertRaises(communityid.FlowTupleError):
            tpl = communityid.FlowTuple(communityid.PROTO_TCP, "ohdear.com", "5.6.7.8")
        with self.assertRaises(communityid.FlowTupleError):
            tpl = communityid.FlowTuple(communityid.PROTO_TCP, "1.2.3.4", "ohdear.com")

        # Need two valid ports
        with self.assertRaises(communityid.FlowTupleError):
            tpl = communityid.FlowTuple(communityid.PROTO_TCP, "1.2.3.4", "5.6.7.8", 23, None)
        with self.assertRaises(communityid.FlowTupleError):
            tpl = communityid.FlowTuple(communityid.PROTO_TCP, "1.2.3.4", "5.6.7.8", None, 23)
        with self.assertRaises(communityid.FlowTupleError):
            tpl = communityid.FlowTuple(communityid.PROTO_TCP, "1.2.3.4", "5.6.7.8", "23/tcp", 23)
        with self.assertRaises(communityid.FlowTupleError):
            tpl = communityid.FlowTuple(communityid.PROTO_TCP, "1.2.3.4", "5.6.7.8", 23, "23/tcp")

        # Need ports with port-enabled protocol
        with self.assertRaises(communityid.FlowTupleError):
            tpl = communityid.FlowTuple(communityid.PROTO_TCP, "1.2.3.4", "5.6.7.8")
    ...
    def test_inputs_py3(self):
        ...
        with self.assertRaises(communityid.FlowTupleError):
            tpl = communityid.FlowTuple(communityid.PROTO_TCP, "1.2.3.4", "5.6.7.8", 23, "80")
...
class LintCommunityID(unittest.TestCase):
    ...
    def test_linting(self):
        ...
        self.assertTrue(out.getvalue().find(" error ") < 0, msg="Pylint error: " + out.getvalue())
...
class TestCommands(unittest.TestCase):
    ...
    def test_communityid(self):
        out = subprocess.check_output([self._scriptpath("community-id"), "tcp", "10.0.0.1", "10.0.0.2", "10", "20"], env=self.env)
    ...
    def test_communityid_pcap(self):
        ...
        self.assertEqual(
            first_line, "1071580904.891921 | 1:LQU9qZlK+B5F3KDmev6m5PMibrg= | 128.232.110.120 66.35.250.204 6 34855 80"
        )
    ...
    def test_communityid_pcap_json(self):
        ...
        self.assertEqual(
            out,
            b'[{"proto": 6, "saddr": "128.232.110.120", "daddr": "66.35.250.204", "sport": 34855, "dport": 80, "communityid": "1:LQU9qZlK+B5F3KDmev6m5PMibrg="}, {"proto": 6, "saddr": "66.35.250.204", "daddr": "128.232.110.120", "sport": 80, "dport": 34855, "communityid": "1:LQU9qZlK+B5F3KDmev6m5PMibrg="}]\n',
        )
    ...
    def test_communityid_tcpdump(self):
        ...
        out = subprocess.check_output(
            [self._scriptpath("community-id-tcpdump")],
            input=b"1071580904.891921 IP 128.232.110.120.34855 > 66.35.250.204.80: Flags [S], seq 3201037957, win 5840, options [mss 1460,sackOK,TS val 87269134 ecr 0,nop,wscale 0], length 0",
            env=self.env,
        )
        ...
        self.assertEqual(
            first_line,
            "1071580904.891921 IP 1:LQU9qZlK+B5F3KDmev6m5PMibrg= 128.232.110.120:34855 > 66.35.250.204.80: Flags [S], seq 3201037957, win 5840, options [mss 1460,sackOK,TS val 87269134 ecr 0,nop,wscale 0], length 0",
        )
